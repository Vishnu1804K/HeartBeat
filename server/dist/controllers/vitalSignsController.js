...
exports.getRecommendations = exports.deleteVitalSign = exports.getVitalSigns = exports.addVitalSign = void 0;
// @desc    Add vital sign
...
const deleteVitalSign = async (req, res) => {
    try {
        const user = req.user;
        const vitalIndex = user.vitalSigns.findIndex((v) => v._id?.toString() === req.params.id);
        if (vitalIndex !== -1) {
...
exports.deleteVitalSign = deleteVitalSign;
...
const getRecommendations = async (req, res) => {
    try {
        ...
        if (user.vitalSigns.length > 0) {
            ...
            const latestBP = bpVitals.sort((a, b) => new Date(b.recordedAt).getTime() - new Date(a.recordedAt).getTime())[0];
            if (latestBP) {
            ...
            const latestHR = hrVitals.sort((a, b) => new Date(b.recordedAt).getTime() - new Date(a.recordedAt).getTime())[0];
            if (latestHR) {
