...
export const unsaveResource = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const user = req.user!;
        const resourceId = req.params.id;
...
export const getSavedResources = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const user = await User.findById(req.user!._id).populate('savedResources');
